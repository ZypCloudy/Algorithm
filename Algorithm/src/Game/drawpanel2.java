...
public class drawpanel2 extends JPanel {
    private int[] circlex = {150, 250, 100, 200, 300, 150, 250};//圆心的x坐标
    private int[] circley = {50, 50, 150, 150, 150, 250, 250};//圆心的y坐标
    private int[] linex1 = {155, 255, 155, 155, 155, 255, 105, 205, 105, 205, 155, 105, 205, 305};//线段起点的x坐标
    private int[] liney1 = {55, 55, 55, 55, 55, 55, 155, 155, 155, 155, 255, 155, 155, 155};//线段起点的y坐标
    private int[] linex2 = {255, 305, 105, 305, 205, 205, 205, 305, 155, 155, 255, 255, 255, 255};//线段终点的x坐标
    private int[] liney2 = {55, 155, 155, 155, 155, 155, 155, 155, 255, 255, 255, 255, 255, 255};//线段终点的y坐标
    private Set<Ellipse2D> circle2 = new HashSet<Ellipse2D>(); //放circle2的哈希表
    private Set<Line2D> line2 = new HashSet<Line2D>();//放line2的哈希表
    private Set<Line2D> visitedline2 = new HashSet<Line2D>();//放visitedline2的哈希表
    private static double x1, y1, x2, y2 = 0;
    private Iterator<Ellipse2D> iterator = circle2.iterator();
    private Iterator<Line2D> iterator1 = line2.iterator();
    private Iterator<Line2D> iterator2 = visitedline2.iterator();


    public drawpanel2() {
        super();
        final JLabel text = new JLabel("关卡二");
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                while (iterator.hasNext())//判断点击的点是否在线段的顶点
                {
                    ...
                    if (e1.contains(x, y)) {
                        ...
                        if (visitedline2.size() == 14)//所有线段访问完毕
                        {
                            visitedline2.clear();
                            x1 = 0;
                            y1 = 0;
    ...
    public Ellipse2D drawcircle(Graphics2D g, int x, int y)//画初始点
    {
        Color c = g.getColor();
    ...
    public void resetcirclecolor(Graphics2D g, int x, int y)//重置点的颜色为蓝色
    {
    ...
    {
        iterator2 = visitedline2.iterator();

        while (iterator2.hasNext() && visitedline2.size() != 0) {

            System.out.println("visited的size为" + visitedline2.size());
    ...
    @Override
    protected void paintComponent(Graphics g)//画初始线段和点
    {
        // TODO Auto-generated method stub
        ...
        circle2.clear();
        for (int i = 0; i < 7; i++) {
            circle2.add(drawcircle(g2, circlex[i], circley[i]));
        ...
        line2.clear();
        for (int i = 0; i < 14; i++) {
            line2.add(drawline(g2, linex1[i], liney1[i], linex2[i], liney2[i]));
            System.out.println("------------------------------------" + line2.size());
        }
